...
# Test blossomos gschema override for errors. If there are no errors, proceed with compiling blossomos gschema, which includes setting overrides.
if [[ -f /usr/share/glib-2.0/schemas/zz0-blossomos-modifications.gschema.override ]]; then
    mkdir -p /tmp/blossomos-schema-test
    find /usr/share/glib-2.0/schemas/ -type f ! -name "*.gschema.override" -exec cp {} /tmp/blossomos-schema-test/ \;
    cp /usr/share/glib-2.0/schemas/zz0-blossomos-modifications.gschema.override /tmp/blossomos-schema-test/
    echo "Running error test for blossomos gschema override. Aborting if failed."
    glib-compile-schemas --strict /tmp/blossomos-schema-test
    echo "Compiling gschema to include blossomos setting overrides"
fi
