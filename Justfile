export repo_organization := env("GITHUB_REPOSITORY_OWNER", "blossomos")
export registry := env("REGISTRY", "registry.blossomos.org")
...
# Build Image
[group('Image')]
build $image="blossomos" $tag="latest" $flavor="main" rechunk="0" ghcr="0" pipeline="0" $kernel_pin="":
    #!/usr/bin/bash
    ...
    LABELS+=("--label" "io.artifacthub.package.readme-url=https://dev.blossomos.org/blossom/os/core/image/-/raw/main/README.md")
    ...
    LABELS+=("--label" "org.opencontainers.image.source=https://dev.blossomos.org/blossom/os/core/image/-/raw/main/Containerfile.in")
    LABELS+=("--label" "org.opencontainers.image.url=https://blossomos.org")
    LABELS+=("--label" "org.opencontainers.image.vendor={{ repo_organization }}")
    LABELS+=("--label" "io.artifacthub.package.deprecated=false")
    LABELS+=("--label" "io.artifacthub.package.keywords=bootc,fedora,blossomos,ublue,universal-blue,kde")
    LABELS+=("--label" "io.artifacthub.package.maintainers=[{\"name\": \"koyu\", \"email\": \"me@koyu.space\"}]")
...
[group('Image')]
build-rechunk image="blossomos" tag="latest" flavor="main" kernel_pin="":
    @{{ just }} build {{ image }} {{ tag }} {{ flavor }} 1 0 0 {{ kernel_pin }}

# Build Image with GHCR Flag
[group('Image')]
build-ghcr image="blossomos" tag="latest" flavor="main" kernel_pin="":
    #!/usr/bin/bash
    if [[ "${UID}" -gt "0" ]]; then
...
[group('Image')]
build-pipeline image="blossomos" tag="latest" flavor="main" kernel_pin="":
    #!/usr/bin/bash
...
rechunk $image="blossomos" $tag="latest" $flavor="main" ghcr="0" pipeline="0":
    ...
    LABELS="
        ...
        io.artifacthub.package.readme-url=https://dev.blossomos.org/blossom/os/core/image/-/raw/main/README.md
        org.opencontainers.image.created=$(date -u +%Y\-%m\-%d\T%H\:%M\:%S\Z)
        org.opencontainers.image.license=Apache-2.0
        org.opencontainers.image.source=https://dev.blossomos.org/blossom/os/core/image/-/raw/main/Containerfile.in
    ...
    if [[ "{{ ghcr }}" == "1" ]]; then
        ...
        ID=$(${SUDOIF} ${PODMAN} images --filter reference={{ registry }}/blossomos/"${base_image_name}":${fedora_version} --format "{{ '{{.ID}}' }}")
        if [[ -n "$ID" ]]; then
            ${PODMAN} rmi "$ID"
        fi
    fi
...
[group('Image')]
load-rechunk image="blossomos" tag="latest" flavor="main":
    #!/usr/bin/bash
    set -eou pipefail
...
# Run Container
[group('Image')]
run $image="blossomos" $tag="latest" $flavor="main":
    #!/usr/bin/bash
    set -eoux pipefail
...
[group('Utility')]
[private]
fedora_version image="blossomos" tag="latest" flavor="main" $kernel_pin="":
    #!/usr/bin/bash
    set -eou pipefail
    {{ just }} validate {{ image }} {{ tag }} {{ flavor }}
...
[group('Utility')]
[private]
image_name image="blossomos" tag="latest" flavor="main":
    #!/usr/bin/bash
    set -eou pipefail
    {{ just }} validate {{ image }} {{ tag }} {{ flavor }}
    if [[ "{{ flavor }}" =~ main ]]; then
...
# Generate Tags
[group('Utility')]
generate-build-tags image="blossomos" tag="latest" flavor="main" kernel_pin="" ghcr="0" $version="" github_event="" github_number="":
    #!/usr/bin/bash
    set -eou pipefail
...
[group('Utility')]
setup-cache $image="blossomos" $tag="latest" $flavor="main" $ghcr="0" $github_event="0":
    #!/usr/bin/bash
    set -eou pipefail
